...
class VoiceLinkProvider(TelephonyProvider):
    ...
    async def handle_websocket(
        ...
        try:
            ...
            # Resolve organization_id for the run
            workflow_run = await db_client.get_workflow_run_by_id(workflow_run_id)
            if not workflow_run:
                logger.error(f"[run {workflow_run_id}] Workflow run not found for VoiceLink pipeline")
                await websocket.close(code=4404, reason="Workflow run not found")
                return
            workflow = await db_client.get_workflow_by_id(workflow_run.workflow_id)
            if not workflow:
                logger.error(f"[run {workflow_run_id}] Workflow not found for VoiceLink pipeline")
